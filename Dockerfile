...
RUN git config --global credential.helper store && \
    ...
    git remote set-url origin https://github.com/zthun/clocks && \
    git remote -v && \
    git checkout latest
...
FROM node:lts-alpine as clocks-web-install
RUN npm install -g @zthun/clocks-web

FROM nginx:stable-alpine as clocks-web
COPY --from=clocks-web-install /usr/local/lib/node_modules/@zthun/clocks-web/dist/. /usr/share/nginx/html/
